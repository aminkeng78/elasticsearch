...
resource "random_string" "mq_admin_user" {
  count   = var.enable && local.mq_admin_user_enabled && ! local.mq_admin_user_is_set ? 1 : 0
  length  = 8
  special = false
  number  = false
}
...
resource "random_password" "mq_admin_password" {
  count   = var.enable && local.mq_admin_user_enabled && ! local.mq_admin_password_is_set ? 1 : 0
  length  = 16
  special = false
}

resource "random_string" "mq_application_user" {
  count   = var.enable && ! local.mq_application_user_is_set ? 1 : 0
  length  = 8
  special = false
  number  = false
}
...
resource "random_password" "mq_application_password" {
  count   = var.enable && ! local.mq_application_password_is_set ? 1 : 0
  length  = 16
  special = false
}

resource "aws_ssm_parameter" "mq_master_username" {
  count       = var.enable && local.mq_admin_user_enabled ? 1 : 0
  name        = format("%s_%s", var.compunent_name, "mq_admin_username")
...
resource "aws_ssm_parameter" "mq_master_password" {
  count       = var.enable && local.mq_admin_user_enabled ? 1 : 0
  name        = format("%s_%s", var.compunent_name, "mq_admin_password")
...
resource "aws_ssm_parameter" "mq_application_username" {
  ...
  name        = format("%s_%s", var.compunent_name, "mq_application_username")
...
resource "aws_ssm_parameter" "mq_application_password" {
  count       = var.enable ? 1 : 0
  name        = format("%s_%s",var.compunent_name, "mq_application_password")
...
resource "aws_mq_broker" "shimq_broker" {
  count                      = var.enable ? 1 : 0
  broker_name                = format("%s_%s", var.compunent_name, "broker") 
   deployment_mode            = var.deployment_mode
  engine_type                = var.engine_type # RabbitMQ
