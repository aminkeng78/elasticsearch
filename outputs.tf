output "broker_id" {
  value       = join("", aws_mq_broker.shimq_broker[*].id)
  description = "AmazonMQ broker ID"
}

output "broker_arn" {
  value       = join("", aws_mq_broker.shimq_broker[*].arn)
